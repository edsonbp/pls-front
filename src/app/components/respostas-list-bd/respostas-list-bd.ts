...
@Component({
  selector: 'app-respostas-list-bd',
  ...
  templateUrl: './respostas-list-bd.html',
  styleUrl: './respostas-list-bd.css',
})
export class RespostasListBd implements AfterViewInit{
    private http = inject(HttpClient);
    private ref = inject(ChangeDetectorRef)	
    protected readonly title = signal('pls-front');
    columns = ['responsavel', 'nomeResponsavel', 'idIndicador',
       'indicador', 'ano', 'mes', 'nomeMes', 'valor', 'protegida' ]
  ...
  gravar(data_block: any){
    let mes_block = data_block.split("/")[0]
    let ano_block = data_block.split("/")[1]
    this.http.get(`http://localhost:8000/gravar?mes_block=${mes_block}&ano_block=${ano_block}`).subscribe((data)=>{
      console.log(data)
    });
  }
}